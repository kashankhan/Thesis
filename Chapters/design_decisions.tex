...
\newpage
	  \begin{figure}[h]
	  	\centering
	  	\includegraphics[width=.7\linewidth]{figures/ch3_critique_algo.png}
	  	\caption{FoodForMe Critiquing Algorithm.}
	  	\label{fig:ch3_critique_algo}
	  \end{figure}	
\newpage
 \begin{algorithm}[H] 
 	\caption{Recipe Critiquing}
 	\label{pseudo_recipe_critiquing}
 	\begin{algorithmic}[1]
 		 \Require{ Recipe Id = $rid$, User Id = $uid$, Star Rating = $s$, List of like ingredients = $li$, List of Dislike ingredients = $di$,  where $li$ and $di$ can be empty or n}
 	   	\State
 		 \Function{RecipeCritique}{$rid, uid, s, li, di$}
	     \State \Call{RateRecipe}{$rid, uid, s$}
	     \State \Call{SetUserIngredientPreference}{$uid, li, true$}
         \State \Call{SetUserIngredientsPreference}{$uid, di, false$}
         \EndFunction
         \State
         \Function{RateRecipe}{$rid, uid, s$}
           \State $r\gets$ \Call{getRecipe}{$rid$}
           \State $u\gets$ \Call{getUserRecipePrefernce}{$uid$}
          \If{($r$ AND $u$ != nil) AND ($u > 0$)}
	         \State $u.rid \gets rid$
	         \State $u.s \gets s$
	        \State recalculate overall star rating of recipe
          \EndIf
          \EndFunction
          \State
          \Require{User Id = $uid$, List of ingredients = $i$, Favorite = $f$}
          \State
          \Function{SetUserIngredientsPreference}{$uid, i, f$}
			\For{Each ingredient $i$}
			\State $u\gets$ \Call{getUserIngredientPreference}{$uid$}
			\State $u.iid\gets ingredient.i$
			\State $u.f\gets f$
			\EndFor
            \EndFunction
 	\end{algorithmic}
 \end{algorithm}
  ...
  \begin{algorithm}[H] 
  	...
  	\begin{algorithmic}[1]
  		\Require{User Id = $uid$ , Cooking Time $ct$, Page Size = $ps$, Course = $c$, List of user like ingredients = $uli$, List of user dislike ingredients = $udli$ }
  		\State
  		\Function{GetUserRecipeRecommendations}{$uid, ct, ps, c$}
			\State $uli\gets$ \Call{GetUserLikeIngredients}{$uid$}
		    \State $udli\gets$ \Call{GetUserDislikeIngredients}{$uid$}
		    \If{$uli.count > 0$ OR $udli.count > 0$}
		    \State $r\gets$ \Call{GetRecommendations}{$uid$}
		    \Else 
		    \State $r\gets$ \Call{GetPopularRecipes}{$ct , ps, c$}
		    \EndIf
		    \State \Return $r$
  	    \EndFunction
  	    \State
  	    \Function{GetRecommendations}{$uid, ct,  ps, c, uli, udli$}
  	       \State $pr\gets$\Call{GetPreferRecipes}{$uli,  c$}
  	        \State Initialize $rr$ as list of recommend recipes
  	      	\For{Each r $pr$}
  	        \State $u\gets$ \Call{getUserIngredientPreference}{$uid$}
  	        \State $tasteMacthes\gets$ \Call{MatchUserTaste}{$r,  udli, ct$}
  	        \If{$tasteMacthes == true$}
	  	    \State  $rr \gets r$
...
Explanation of recommended item depends on: (1) How much calories are contained by recommended recipe (2) How much user needs to do exercise to burn such amount of calories. . (3) Factors why this particular recipe in user’s recommended item set. Let’s discuss each item hypothesis behind each of discuss item. Calories are important factor that person has to major if he wants to live healthier. To deal with health factor we have to provide calories information so that he can keep track of his consumption of calories. After given information about calories it is also important to highlight how much he have to workout to burn calories. Idea behind this to integrate app with another health or medical app and based on his health condition recommend him recipes. Moreover, it is also important to keep aware of user why system is recommending him such item. For this reason we consider user preferences about recipe and ingredients. More specifically every recipe which is recommended to user contains atleast one of his favorite ingredient. Additionally organized by cooking time which is provided by user according to his preference. Figure \ref{fig:ch3_explanation_generator} is a graphical representation how our system will generate recommendation’s explaination.
  ...
  \end{figure}
    \begin{algorithm}[H] 
    	\caption{Generating recipe explanation}
    	\label{pseudo_recipe_explanation}
    	\begin{algorithmic}[1]
    		\Require{User Id = $uid$, Recipe = $r$ , User Cooking Time $uct$, List of user like ingredients = $uli$.}
    		\State 
    		\Function{GetUserRecipeExplanation}{$uid, r, ct$}
    	    \State Initialize $rr$ as recommended recipe
    	      \State $rr.calories \gets ct$
    	      \State $rr.calories \gets$\Call{GetRecipeCalorie}{$r$}
    	      \State $rr.exerciseTime \gets$\Call{GetCalorieBurningTime}{$uid, rr.calories$}.  \Comment{This method calculate ratio between the user’s BMI rate and recipe’s calories and returns the amount of time in mins, how much user need to do exercise to achieve his health target.}
    			\For{Each ${R}_i$ $r.ingredients$}
    			   \For{Each ${U}_i$ $uli$}
	    			   \If {${R}_i == {U}_i$}
	    			   \State $rr.addPreferIngredient \gets {R}_i$ \Comment{$rr.preferIngredients$ is a list of prefer Ingredients}
				  	   \EndIf
    			    \EndFor
    			\EndFor
    			\Return $rr$
