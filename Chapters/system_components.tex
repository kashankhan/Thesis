...
As discuss in earlier section, user context have a high impact while recommendations are made.  Briefly user context refers to the current activities of user for example, what user is looking for under what circumstances. According to Lee, H. et al.,’s classification \cite{lee2005context} it is “Identity”.  In our system \textit{Recipe’s Cousin} is considering as a user’s interest. Since all the recipes are categories by cousin example, drinks, breakfasts and appetizer. User has to choose cousin in order to define his interest. 
